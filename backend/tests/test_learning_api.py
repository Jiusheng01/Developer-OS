...
def test_learning_item_crud_flow(client: TestClient) -> None:
    ...
    list_response = client.get("/api/v1/learning-items")
    ...
    delete_response = client.delete(f"/api/v1/learning-items/{created['id']}")
    assert delete_response.status_code == 204
    assert client.get("/api/v1/learning-items").json() == []


def test_learning_item_defaults_and_validation(client: TestClient) -> None:
    response = client.post("/api/v1/learning-items", json={"title": "Docker"})
    ...
    blank_response = client.post("/api/v1/learning-items", json={"title": "   "})
...
def test_missing_learning_item_returns_404(client: TestClient) -> None:
    response = client.patch("/api/v1/learning-items/missing", json={"progress": 20})
